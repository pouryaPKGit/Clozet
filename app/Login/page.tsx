...
const AuthForm: React.FC = () => {
  ...
  return (
    <div className="h-[100vh] items-center flex bg-[#faebd4] justify-center px-5 lg:px-0">
      <div className="max-w-screen-xl bg-[#b88c68] shadow-md shadow-white rounded-2xl flex justify-center flex-1">
        {isSignup ? (
          <>
            <div className={`flex-1 rounded-2xl text-center hidden md:flex ${imageClass}`}>
              <div className="m-12 xl:m-16 w-full bg-contain bg-center bg-no-repeat">
                <Image
                  src="/images/Untitled-1.png21112.png"
  ...
  return (
    <form className="flex flex-col items-center justify-center" onSubmit={handleSubmit}>
      <div className="text-center">
        <h1 className="text-2xl xl:text-4xl font-extrabold text-[#faebd4]">
          {isSignup ? 'فرم ثبت نام' : 'فرم ورود'}
        </h1>
        <p className="text-[12px] text-[#faebd4] font-bold mt-4 text-md">
      ...
      <div className="w-full flex-1 mt-8">
        <div className="mx-auto max-w-xs flex flex-col gap-4">
          {isSignup && (
            <input
              dir="rtl"
              className="w-full px-5 py-3 rounded-lg font-medium bg-[#faebd4] border-2 border-white placeholder-gray-800 text-sm focus:outline-none focus:border-gray-400 text-right"
              type="text"
          ...
          <button
            type="submit"
            className="mt-5 w-full px-5 py-1 text-lg   bg-[#b46f37] text-white hover:opacity-65 duration-300  rounded-lg font-medium "
          >
            {isSignup ? 'ثبت نام' : 'ورود'}
          </button>
          <p className="mt-3 text-sm text-[#faebd4] flex items-center justify-center">
            {isSignup ? (
              <>
                آیا حساب کاربری دارید؟{' '}
                <button type="button" className="text-[#faebd4] font-bold pr-1" onClick={() => setIsSignup(false)}>
                  ورود به حساب
                </button>
              </>
            ) : (
              <>
                حساب کاربری ندارید؟{' '}
                <button type="button" className="text-[#faebd4] font-bold pr-1" onClick={() => setIsSignup(true)}>
                  ثبت نام
                </button>
              </>
            )}
