...
const Page: React.FC = () => {
  ...
  return (
    <>
      <div>
        <h1 className='bg-[#faebd4] text-black w-[150px] h-[40px] mt-10  rounded-md font-semibold mr-28 text-md flex items-center justify-center'>تمامی محصولات </h1>
      </div>
      <div className="pt-16 flex flex-wrap justify-center gap-8 px-5">
        {currentItems.map((item: Product, index: number) => (
          <div key={index} className="bg-[#faebd4] rounded-2xl shadow-white shadow-md p-3 w-[300px] h-[360px] group relative overflow-hidden">
            <Link href={`/product/${item.id}`}>
              <div className="flex flex-col gap-y-2 items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-gray-900 font-semibold">{item.price} تومان</span>
                </div>
              </div>
            </Link>
          </div>
      ...
      <div className="flex justify-center mt-10">
        ...
        <span className="px-4 py-2 text-gray-900">{currentPage} از {totalPages}</span>
        <button
      ...
      <div>
        <h1 className=' text-black w-[170px] h-[40px] mt-20 rounded-md font-semibold mr-28 text-md flex items-center justify-center bg-[#faebd4]'>پر فروش های مجموعه</h1>
      </div>
      <div className="mt-10 mb-16 w-[86%] mx-auto flex gap-10 bg-[#faebd4] rounded-xl">
        <MainSlider />
      </div>
    </>
  );
